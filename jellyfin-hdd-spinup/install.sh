...
# True only if a watcher instance is actually alive. PID-aware, so a stale lock
# left by a hard kill or an unclean shutdown is not mistaken for a running app.
running() {
  [ -f "$PIDFILE" ] || return 1
...
# Already running? (PID-aware; a stale lock does not count as running.)
if [ -f "$PIDFILE" ]; then
  p="$(cat "$PIDFILE" 2>/dev/null)"

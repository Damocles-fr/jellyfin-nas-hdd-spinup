...
# ---------------------------------------------------------------------------
# True only if PID is alive AND is actually one of our watcher processes.
# Checking the command line guards against PID reuse making a dead watcher look
# "alive" (which would otherwise block a restart and prevent self-healing).
is_alive_watcher() {
  _p="$1"
  [ -n "$_p" ] || return 1
  kill -0 "$_p" 2>/dev/null || return 1
  if [ -r "/proc/$_p/cmdline" ]; then
    tr '\0' ' ' < "/proc/$_p/cmdline" 2>/dev/null | grep -q 'spinup_ws_login.sh'
    return $?
  fi
  return 0
}
...
acquire_lock() {
  ...
  if is_alive_watcher "$oldpid"; then
    return 1
  fi
...
# ---------------------------------------------------------------------------
start_tailproc() {
  # Stop the previous worker AND its tail child before starting a new one.
  #
  # BusyBox ash specifics that matter here:
  #   - the "tail" is a SEPARATE child of this shell, so killing the
  #     while-subshell ($TAILPID) does NOT kill the tail; we must sweep it.
  #   - "wait" on the pipeline pid blocks until the WHOLE job (tail included)
  #     has exited. Because the tail keeps running, that wait would hang the
  #     watcher forever on a log rotation. So we never wait on the pipeline.
  if [ -n "$TAILPID" ]; then
    kill "$TAILPID" 2>/dev/null
    TAILPID=""
  fi
  # Sweep the tail following our log directory (our own tail child, plus any
  # orphan tail left over from a previous run or a rotation).
...
while :; do
  ...
  # Self-heal: (re)start the tail worker if a newer log file appeared OR if the
  # current worker has died for any reason. Without this we could get stuck
  # following an old, no-longer-written log file after a rotation.
  worker_dead=0
  if [ -n "$CURRENT_FILE" ]; then
    if [ -z "$TAILPID" ] || ! kill -0 "$TAILPID" 2>/dev/null; then
      worker_dead=1
    fi
  fi

  if [ -n "$LATEST" ] && { [ "$LATEST" != "$CURRENT_FILE" ] || [ "$worker_dead" = "1" ]; }; then
    CURRENT_FILE="$LATEST"
    start_tailproc
  fi
  sleep "$SLEEP"
